...
- Repository：`guardrail-mcp`（GitHub: cawa0505/guardrail-mcp）
- 本地 Go module / binary / 目錄名稱：已統一為 `guardrail-mcp` / `GuardrailMcp`
- 核心實作語言：**Go**（已上線）
...
- **MR-1 命名統一**：remote repository、本地 Go module、binary、目錄名稱已統一為 `guardrail-mcp` / `GuardrailMcp`。
