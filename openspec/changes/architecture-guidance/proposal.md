...
StateMachineMcp 的產品定位與架構邊界目前不明確。對外定位是 Guardrail MCP，但實作語言（Go）與未來方向（Rust）混雜、Graphify 的角色模糊、雙軌驗證（Hard Guard / Soft Guard）缺乏定義。本變更釐清這些邊界，確保架構文件與產品事實一致。

**產品事實：**
- 對外定位：Guardrail MCP — 透過 phase gate、policy 驗證、compiler/linter 與 code graph 來保護編碼工作流程
- Remote repository：改名為 `guardrail-mcp`
- 本地 Go module / binary / 目錄名稱：維持 `statemachine-mcp` / `StateMachineMcp`，不在本變更改名
- 核心實作語言：**Go**（已上線）
...
- **MR-1 雙命名策略**：remote repository 改名為 `guardrail-mcp`；本地 Go module、binary、目錄名稱維持 `statemachine-mcp` / `StateMachineMcp`，不在本變更改名。
